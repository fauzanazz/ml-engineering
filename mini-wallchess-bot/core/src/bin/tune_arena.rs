...
fn main() {
    ...
    let a_cfg = BotConfig::new(Heuristic { w_path: wpa, w_wall: wwa }, da);
    let b_cfg = BotConfig::new(Heuristic { w_path: wpb, w_wall: wwb }, db);

    let (mut aw, mut bw, mut draws) = (0u32, 0u32, 0u32);
    for gm in 0..games {
        let a_south = gm % 2 == 0;
        let (south, north) = if a_south { (&a_cfg, &b_cfg) } else { (&b_cfg, &a_cfg) };
