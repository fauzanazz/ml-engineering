...
def test_validate_provider_boundaries_allows_adapter_imports(tmp_path: Path) -> None:
    adapter_path = tmp_path / "configs" / "platform" / "adapters" / "aws" / "adapter.py"
    core_path = tmp_path / "artifacts" / "reports" / "production-patterns" / "workflow.py"
...
def test_validate_provider_boundaries_rejects_core_vendor_imports(tmp_path: Path) -> None:
    source_path = tmp_path / "artifacts" / "reports" / "production-patterns" / "workflow.py"
    ...
    assert report["violations"] == ["artifacts/reports/production-patterns/workflow.py"]
