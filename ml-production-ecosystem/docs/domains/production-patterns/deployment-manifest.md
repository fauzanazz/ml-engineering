...
`registry_path` records `registry/models.json`, the local registry file that stores registered versions and active model state.
...
Update `configs/production-patterns/deploy/deployment-manifest.yaml` whenever service name, image name, command, port, endpoint path, registry path, release checklist path, or rollback procedure changes.
