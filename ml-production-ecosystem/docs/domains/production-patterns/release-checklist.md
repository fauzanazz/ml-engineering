...
- [ ] Check current active model in `registry/models.json` and record known-good version for rollback.
...
- [ ] Confirm `artifacts/reports/production-patterns/scheduled-retraining.json` has `status: "completed"`.
...
- [ ] Confirm `registry/models.json` active entry points to intended version.
...
- [ ] Confirm `artifacts/reports/production-patterns/rollback.json` records reason and target version.
