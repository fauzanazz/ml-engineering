  ...
  return (
    <main className="page-wrap flex min-h-[70vh] flex-col items-center justify-center px-4 py-14">
      <div className="w-full max-w-2xl text-center">
        ...
        <h1 className="display-title mb-8 text-4xl font-bold text-foreground sm:text-5xl">
          Who are you playing?
        </h1>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {MODES.map(({ mode, icon: Icon, title, body }) => (
            <Link
              ...
              to="/game"
              search={{ mode }}
              className="border bg-card text-card-foreground shadow-sm transition hover:-translate-y-px hover:border-foreground/25 rise-in flex flex-col items-center gap-3 rounded-xl p-7 text-center no-underline"
            >
        ...
        <Link
          to="/"
          className="mt-8 inline-block text-sm font-semibold text-muted-foreground"
        >
          ← Back
        </Link>
      </div>
