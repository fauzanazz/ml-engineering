...
from webcam_effect.yolo_models import YoloFrameClassifier, YoloPersonDetector
...
def run_live_effect(
    ...
    effect_definition = load_effect_definition(Path(effect_config_path) if effect_config_path else None)
    ...
    effect_audio = audio_path if audio_path else effect_definition.audio
    ...
    audio = LoopingAudio(Path(effect_audio))
    ...
    try:
        while True:
            ...
            output = effect.apply(frame) if effect_active else frame
