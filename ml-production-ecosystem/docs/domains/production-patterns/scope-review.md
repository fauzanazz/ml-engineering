...
- Step 12 added the `production-patterns domain` scaffold, docs area, and package boundary for production patterns.
...
Recommended next module: `scale-reliability domain`.
...
1. Start `scale-reliability domain` with a scope document and first reliability requirement.
...
Recommended choice: start `scale-reliability domain` only if the user wants to move beyond local production patterns into scale and reliability.
