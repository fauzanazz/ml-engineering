...
def run_load_test(
    ...
    if concurrency == 1:
        results = [_safe_request(send_request, request_url, timeout_seconds) for _ in range(request_count)]
    else:
        worker_count = min(concurrency, request_count)
        with ThreadPoolExecutor(max_workers=worker_count) as executor:
            futures = [executor.submit(_safe_request, send_request, request_url, timeout_seconds) for _ in range(request_count)]
...
def _safe_request(send_request: Requester, url: str, timeout_seconds: float) -> tuple[int | None, float]:
    try:
        return send_request(url, timeout_seconds)
    except Exception:
        return None, 0.0
