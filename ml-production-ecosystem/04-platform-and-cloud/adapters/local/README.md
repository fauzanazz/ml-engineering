...
`adapter.py` implements `LocalProviderAdapter`, which returns filesystem resources and env-var secret references through `shared.platform.InfrastructurePlan`.

It does not read cloud credentials, resolve secret values, or import provider SDKs.
