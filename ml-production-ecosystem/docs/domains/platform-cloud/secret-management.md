...
`configs/platform/secrets/local/secret-injections.yaml` lists required env-var targets and policy references for local runs. It never stores secret values; `value_handling: external-only` means users provide values outside git through their shell, ignored `.env`, or local secret tooling.
